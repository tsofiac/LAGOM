...
def metxbiodb_inchi_to_smiles(data, output_file):
    ...
    for ind in data.index:
        try:
            ...
            enzyme = data.iloc[ind]["enzyme"]

             # Check if enzyme is empty
            if not enzyme or str(enzyme).strip() == "":
                print(f"Warning: Enzyme information is missing for index {ind}")
                enzyme = "UNKNOWN"
    ...
    smiles_metxbiodb_df.columns = ['parent_name', 'parent_smiles', 'child_name', 'child_smiles', 'enzyme']
